...
<?php $__env->startSection('title', 'Katalog - Jaya Maju Yk'); ?>
...
<section id="catalog" class="max-w-[1200px] mx-auto px-6 pb-20 pt-10 min-h-screen">
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12">
        ...
        <div class="flex items-center gap-3 bg-white p-2 rounded-2xl border border-gray-100 shadow-sm overflow-x-auto snap-x scroll-pl-6 no-scrollbar pb-2 md:pb-2">
            <a href="<?php echo e(route('catalog')); ?>" class="snap-start shrink-0 px-6 py-4 md:py-3 rounded-xl text-xs font-black uppercase transition-all active:scale-95 <?php echo e(!request('category') ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-slate-400 hover:text-primary hover:bg-gray-50'); ?>">Semua</a>
            <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <a href="<?php echo e(route('catalog', ['category' => $cat])); ?>" class="snap-start shrink-0 whitespace-nowrap px-6 py-4 md:py-3 rounded-xl text-xs font-black uppercase transition-all active:scale-95 <?php echo e(request('category') === $cat ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-slate-400 hover:text-primary hover:bg-gray-50'); ?>">
                    <?php echo e($cat); ?>
    ...
    <?php if($products->isEmpty()): ?>
        <div class="text-center py-40 bg-white rounded-[4rem] border border-gray-100 shadow-sm">
            ...
            <a href="<?php echo e(route('catalog')); ?>" class="mt-10 px-10 py-5 bg-primary text-white rounded-2xl font-black uppercase tracking-widest text-xs hover:scale-105 transition-all shadow-xl shadow-primary/20 inline-block active:scale-95">
                Reset Filter
            </a>
        </div>
    <?php else: ?>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
            ...
            <div class="group bg-white rounded-3xl border border-slate-100 p-4 transition-all duration-300 hover:border-primary/20 hover:shadow-2xl hover:shadow-primary/5 hover:-translate-y-1 flex flex-col h-full active:border-primary/50">
                ...
                <div class="relative aspect-[4/3] rounded-2xl overflow-hidden bg-slate-50 mb-6 group/image">
                    <?php if($product->images && count($product->images) > 1): ?>
                        <div class="flex overflow-x-auto snap-x snap-mandatory h-full no-scrollbar scroll-smooth">
                            <?php $__currentLoopData = $product->images; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $img): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <img 
                                    src="<?php echo e(Str::startsWith($img, 'assets') ? asset($img) : $img); ?>" 
                    ...
                    <a href="<?php echo e(route('product.show', $product->id)); ?>" class="absolute inset-0 z-10"></a>
                </div>
                ...
                <div class="flex-grow space-y-4 px-2">
                    ...
                    <div class="flex items-center gap-2">
                        <span class="text-primary font-black text-xl">Rp <?php echo e(number_format($product->price, 0, ',', '.')); ?></span>
                        <?php if($product->original_price): ?>
                            <span class="text-slate-300 line-through text-sm decoration-2">Rp <?php echo e(number_format($product->original_price, 0, ',', '.')); ?></span>
                        <?php endif; ?>
                    </div>
                </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\JAYA Maju\resources\views/catalog.blade.php ENDPATH**/ ?>